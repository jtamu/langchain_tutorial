...
class Interview(BaseModel):
    ...
    question: str = Field(..., description="インタビューでの質問内容")
    answer: str = Field(..., description="インタビューでの回答内容")
...
class EvaluationResult(BaseModel):
    ...
    is_sufficient: bool = Field(..., description="情報が十分かどうかの判定")


class InterviewState(BaseModel):
    user_request: str = Field(..., description="ユーザーのリクエスト内容")
    ...
    is_information_sufficient: bool = Field(
        default=False, description="情報が十分かどうかの判定"
    )
    ...
    def _generate_questions(
        ...
        question_prompt = ChatPromptTemplate.from_messages(
            [
                ...
                (
                    "human",
                    "以下のペルソナに関連するユーザーリクエストについて、１つの質問を生成してください。\n\n"
                    "ユーザーリクエスト：{user_request}\n"
...
class RequirementsdDocumentGenerator:
    def __init__(self, llm: ChatOpenAI):
        self.llm = llm
...
class DocumentationAgent:
    def __init__(self, llm: ChatOpenAI, k: Optional[int] = None):
        ...
        self.requirements_generator = RequirementsdDocumentGenerator(llm=llm)
    ...
    def _generate_requirements(self, state: InterviewState) -> dict[str, Any]:
        requirements_doc = self.requirements_generator.run(
            state.user_request,
            state.interviews,
        )
